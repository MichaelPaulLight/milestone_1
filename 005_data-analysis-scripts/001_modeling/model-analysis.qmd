...
The second plot, which visualizes intercepts output by the model, helps identify counties that have particularly high or low baseline vote percentages, after accounting for other factors in the model. It's useful for understanding regional variations in voting behavior that aren't explained by the other predictors in the model.

```{r}
# Creating a plot for County-Level Staff Rating Effects
model |>
  # Extract posterior samples for staff rating effect and county-specific random effects
  spread_draws(b_staff_rating, r_county[county,]) |>
  
  # Calculate county-specific staff rating effect
  ...
  # Add half-eye plot (combination of density and point estimate)
  stat_halfeye() +
  
  # Add vertical reference lines at -0.8 and 0.8
  geom_vline(xintercept = c(-.8, .8), linetype = "dashed", color = "#2c3e50", alpha = 0.7) +
  
  # Set custom colors for the fill based on whether effect is within -0.8 to 0.8
  ...
  # Adjust text size for county names on y-axis
...
# Function to create a styled plot for each effect
plot_effect <- function(effect_data, effect_name) {
  ggplot(effect_data, aes(x = .data[[effect_name]], y = estimate__)) +
    geom_ribbon(aes(ymin = lower__, ymax = upper__), fill = report_colors[1], alpha = 0.3) +
    geom_line(color = report_colors[2], size = 1) +
    labs(
      title = paste("Effect of", gsub("_", " ", effect_name)),
      x = gsub("_", " ", effect_name),
      y = "Predicted Vote Percentage"
    ) +
    scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
    theme_report() +
...
plots <- map2(ce, names(ce), plot_effect)

# Combine plots using patchwork
combined_plot <- wrap_plots(plots, ncol = 2) +
  plot_annotation(
    title = "Conditional Effects of Model Parameters",
