...
```{r}
param_names <- c(
  b_mortality_rate_facility = "Mortality Rate",
  b_n_dialysis_stations = "Number of Dialysis Stations",
  b_five_star = "Five Star Rating",
  b_patient_experience_rating = "Patient Experience Rating",
  b_staff_rating = "Staff Rating"
)

mcmc_areas(
  model, 
  pars = c("b_mortality_rate_facility", "b_n_dialysis_stations", 
           "b_five_star", "b_patient_experience_rating", "b_staff_rating"),
  prob = 0.95,  # 95% credible interval
  prob_outer = 0.99  # 99% credible interval
) +
  scale_fill_manual(values = report_colors[1:5]) +
  scale_y_discrete(labels = param_names) +  # Use readable parameter names
  labs(
    title = "Posterior Distributions of Model Parameters",
    subtitle = "Shaded areas represent 95% and 99% credible intervals",
    x = "Parameter Estimate",
    y = NULL  # Remove y-axis label as parameter names are self-explanatory
  ) +
  theme_report() +
  theme(
    legend.position = "none",
